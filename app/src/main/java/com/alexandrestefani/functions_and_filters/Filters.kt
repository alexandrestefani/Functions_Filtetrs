package com.alexandrestefani.functions_and_filters
...
fun main() {
    ...
    val testassociateby =
        listanomes.associateBy { it.length } //retorna apenas o último registro com o mesmo tamanho
    ...
    val novosnomes =
        listanomes.map {
            "$it, voce está bem ?"
        }.zip(listanumeros) { texto, num ->
            "$texto, voce é a pessoa numero: $num"
        }.forEach { println(it) }
}